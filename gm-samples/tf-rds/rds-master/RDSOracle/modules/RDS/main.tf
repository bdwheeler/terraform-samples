...
resource "aws_db_subnet_group" "group" {
  ...
  tags = {
    Name = "${lower(var.db_name)} DB subnet group"
...
resource "aws_security_group" "db_sg" {
  ...
  description = "${lower(var.db_name)} Database security group"
  ...
  tags = {
    Name = "${lower(var.db_name)} Database Security Group"
    Author = "Effectual Terraform script"
    Date = "${timestamp()}"
  }
}
...
resource "aws_db_instance" "new_db" {
  ...
  tags = {
          Name = "${join("-", ["${lower(var.db_name)}", "${var.client}", "RDS"])}"
...
resource "aws_db_parameter_group" "default" {
  ...
  description = format("Parameter group for %s", lower(var.db_name))
  family      = "${var.family}"
  ...
  parameter {
    ...
    value = var.enable_goldengate_replication
  }
...
resource "aws_db_option_group" "without_s3_integration" {
  ...
  option_group_description = format("Option group for %s", lower(var.db_name))
